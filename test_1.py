...
if selected == "Admin":
            ...
            with connection.cursor(pymysql.cursors.DictCursor) as cursor:
                ...
                if data:
                    ...
                    if not filtered_data:
                        st.warning(f"⚠️ No suitable data found for Student ID {student_id}.")
                        st.session_state.attendance_data = None
                    else:
    ...
    if st.session_state.attendance_data is not None:
        ...
        # Display badges only for eligible students
        st.write("### 🎉 Achievements: 🎊")
        eligible_students = [
            row for _, row in st.session_state.attendance_data.iterrows() if row['days_present'] >=1
        ]

        if eligible_students:
            badge_columns = st.columns(len(eligible_students))
            for idx, row in enumerate(eligible_students):
                badge, caption = assign_badge(row['days_present'], row['month'])
                if badge:
                    with badge_columns[idx]:
                        st.image(badge, caption=caption, width=120)
        else:
            st.warning("⚠️ No eligible attendance records for badges.")
        try:
